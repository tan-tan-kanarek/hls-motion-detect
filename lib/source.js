...
class Source extends EventEmitter {
	...
	constructor({systemName, sourceURL, sourcePath, ffmpegPath, ffprobePath}){
		super();
	...
	processVideo(filepath){
		let This = this;
		return new Promise((resolve, reject) => {
    		let cmd = `${This.ffprobePath} -show_frames -print_format json ${filepath}`;
...
//					console.log(diff);
	...
	captureImage(videoPath, imagePath, keyFrame){
		let This = this;
		return new Promise((resolve, reject) => {
			let cmd = `${This.ffmpegPath} -i ${videoPath} -ss ${keyFrame} -vframes 1 ${imagePath}`;
			cmd.exec((stdout, stderr) => {
				This.emit('capture', imagePath);
				resolve();
    		});
		});
	}
